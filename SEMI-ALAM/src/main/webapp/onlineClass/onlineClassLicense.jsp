			...
			<c:choose>
				...
				<c:otherwise>
					<c:forEach items="${list }" var="dto" varStatus="status">
						...
						<td class="video"><video src="./${dto.upload_file_name }"></video>
							<div class="info">
								글 제 목 : <span class="title">${dto.upload_title }</span><br /> 작
								성 자 : <span class="id">${dto.upload_id }</span><br /> 결 제 액 : <span
									class="price">${dto.upload_price }</span>원<br />
								...
								<c:if test="${user != null && user.member_id == dto.upload_id}">
									<input type="button" onclick="deleteVideo('${dto.upload_no }')"
