...
<script src="https://semi-video.herokuapp.com/socket.io/socket.io.js"></script>
...
<body>
	...
	<div class="wrapper">
		...
		<div class="user-container">
			...
			<input type="text" id="member_Id" value="${user.member_id}" readonly /> 
				
			<label for="guest">To : </label> 
			
			<input type="text" id="guest_Id" value="${guest_Id}" readonly />
		</div>
			...
			<c:choose>
				...
				<c:otherwise>
					<c:forEach items="${msgList }" var="dto">
						<li class=${dto.from_id == user.member_id ? 'sent' : 'received' }>						
							<span class="profile">
		        				<span class="user">${dto.from_id}</span>
						        <img
						          class="image"
		      				...
		      				<span class="time"><fmt:formatDate value="${dto.send_date}" pattern="yyyy-MM-dd h:mm:ss a"/></span>
	      				</li>
					</c:forEach>
