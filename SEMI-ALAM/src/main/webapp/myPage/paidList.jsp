...
<body>
	...
	<div class="container">
		...
		<div class="content">
			<h1>온라인 강의 ${user.member_grade == 'S' ? '판매' : '구매' } 목록</h1>
			...
			<table class="table_all" border="1">
				...
				<tr>
					<th>강의 번호</th>
					<th>${user.member_grade == 'S' ? '구매자' : '판매자' } ID</th>
					<th>강의 제목</th>
					<th>강의 가격</th>
					<th>강의 카테고리</th>
					<th>구매한 날짜</th>
				...
				<c:choose>
					...
					<c:otherwise>
						<c:forEach items="${list }" var="dto">
							<tr>
								<td>${dto.class_no }</td>
								<td>${user.member_grade == 'S' ? dto.buyer_id : dto.seller_id }</td>
			...
			<c:if test="${user.member_grade == 'S' }">
				...
				<table class="table_all" border="1">
					...
					<c:choose>
						...
						<c:otherwise>
							<c:forEach items="${paidList }" var="dto">
								<tr>
									<td>${dto.class_no }</td>
									<td>${dto.seller_id }</td>
									<td>${dto.class_title }</td>
